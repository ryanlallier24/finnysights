...
const CommentItem = ({ comment, currentUser, avatarMap }) => {
  ...
  return (<div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700/50"><div className="flex items-start gap-2"><AvatarDisplay avatar={commentAvatar} size="sm" /><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className="text-sm font-bold text-white">{comment.displayName}</span><span className="text-[10px] text-slate-500">{timeAgo(comment.createdAt)}</span></div><p className="text-xs text-slate-300 leading-relaxed">{comment.content}</p><button onClick={handleLike} className={`mt-2 text-[10px] flex items-center gap-1 transition-colors ${liked ? 'text-rose-400' : 'text-slate-500 hover:text-rose-400'}`}><Heart size={10} className={liked ? 'fill-rose-400' : ''} />{likesCount}</button></div></div></div>);
};
