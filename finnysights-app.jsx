...
const CommentItem = ({ comment, currentUser, avatarMap }) => {
  ...
  const handleReaction = async (emoji) => {
    ...
    setLocalReactions(prev => {
      const updated = { ...prev };
      const uids = updated[emoji] || [];
      if (uids.includes(currentUser.uid)) {
        updated[emoji] = uids.filter(id => id !== currentUser.uid);
        if (updated[emoji].length === 0) delete updated[emoji];
      } else {
        updated[emoji] = [...uids, currentUser.uid];
      }
      return updated;
    });
    await reactToComment(currentUser.uid, comment.id, emoji);
  };
  ...
  const commentAvatar = (avatarMap && avatarMap[comment.uid]?.avatar) || comment.avatar || getAvatar(comment.uid);
  const reactionEntries = Object.entries(localReactions).filter(([, uids]) => uids && uids.length > 0);
  return (
    <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700/50 group relative">
      <div className="flex items-start gap-2">
        <AvatarDisplay avatar={commentAvatar} size="sm" />
        <div className="flex-1">
              ...
              return (
                <button key={emoji} onClick={() => handleReaction(emoji)}
                  className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] transition-all ${
                    hasReacted ? 'bg-cyan-500/20 border border-cyan-500/30 text-cyan-300' : 'bg-slate-800/80 border border-slate-700/50 text-slate-400 hover:border-slate-600'
                  }`}
                >
                  <span>{emoji}</span><span className="font-bold">{uids.length}</span>
                </button>
              );
            })}
            {/* Add reaction button */}
            {currentUser && (
              <div className="relative">
                <button onClick={() => setShowPicker(!showPicker)}
                  className="w-5 h-5 flex items-center justify-center rounded-full bg-slate-700/50 hover:bg-slate-600 border border-slate-600/50 transition-all opacity-0 group-hover:opacity-100"
                >
                  <SmilePlus size={10} className="text-slate-400" />
                </button>
                {showPicker && (
                  <div className="absolute left-0 bottom-7 z-20 flex gap-0.5 px-1.5 py-1 bg-slate-800 border border-slate-600/50 rounded-full shadow-xl">
                    {REACTION_EMOJIS.map(emoji => (
                      <button key={emoji} onClick={() => handleReaction(emoji)}
                        className={`w-6 h-6 flex items-center justify-center rounded-full text-sm hover:bg-slate-700 hover:scale-125 transition-all ${localReactions[emoji]?.includes(currentUser?.uid) ? 'bg-slate-700/80' : ''}`}
                      >{emoji}</button>
                    ))}
                  </div>
                )}
...
const DetailPanel = ({ item, onClose, userVote, isLoading, currentUser, comments, onAddComment, onRefreshComments, isCrypto, news, isLoadingNews, commentAvatarMap }) => {
...
const GlobalChat = ({ currentUser, userProfile }) => {
  ...
  const handleReaction = async (messageId, emoji) => {
    if (!currentUser) return;
    setPickerMsgId(null);
    await reactToGlobalMessage(currentUser.uid, messageId, emoji);
  };
        ...
        <>
          ...
          <div ref={chatContainerRef} className="h-64 overflow-y-auto px-4 space-y-3 scrollbar-thin">
            {messages.length === 0 ? (
              ...
              const reactionEntries = Object.entries(reactions).filter(([, uids]) => uids && uids.length > 0);
              return (
              <div key={msg.id} className="relative group" onMouseEnter={() => setHoveredMsgId(msg.id)} onMouseLeave={() => { setHoveredMsgId(null); if (pickerMsgId === msg.id) setPickerMsgId(null); }}>
                <div className={`flex gap-2 ${isOwn ? 'flex-row-reverse' : ''}`}>
                  <AvatarDisplay avatar={msg.avatar} size="sm" />
                  <div className={`max-w-[75%] ${isOwn ? 'text-right' : ''}`}>
                    <div className="flex items-center gap-1.5 mb-0.5" style={{ justifyContent: isOwn ? 'flex-end' : 'flex-start' }}>
                    ...
                    <div className="relative inline-block">
                      ...
                      {currentUser && hoveredMsgId === msg.id && (
                        <button
                          onClick={(e) => { e.stopPropagation(); setPickerMsgId(pickerMsgId === msg.id ? null : msg.id); }}
                          className={`absolute ${isOwn ? '-left-7' : '-right-7'} top-0 w-5 h-5 flex items-center justify-center rounded-full bg-slate-700/80 hover:bg-slate-600 border border-slate-600/50 transition-all opacity-0 group-hover:opacity-100`}
                        >
                          <SmilePlus size={10} className="text-slate-300" />
                        </button>
                      )}
                      {/* Emoji picker */}
                      {pickerMsgId === msg.id && (
                        <div className={`absolute ${isOwn ? 'right-0' : 'left-0'} -top-9 z-20 flex gap-0.5 px-1.5 py-1 bg-slate-800 border border-slate-600/50 rounded-full shadow-xl`}>
                          {REACTION_EMOJIS.map(emoji => (
                            <button
                              key={emoji}
                              onClick={() => handleReaction(msg.id, emoji)}
                              className={`w-6 h-6 flex items-center justify-center rounded-full text-sm hover:bg-slate-700 hover:scale-125 transition-all ${reactions[emoji]?.includes(currentUser?.uid) ? 'bg-slate-700/80' : ''}`}
                            >
                              {emoji}
                            </button>
                          ))}
                      ...
                      <div className={`flex flex-wrap gap-1 mt-1 ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        {reactionEntries.map(([emoji, uids]) => {
                          ...
                          return (
                            <button
                              key={emoji}
                              onClick={() => currentUser && handleReaction(msg.id, emoji)}
                              className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] transition-all ${
                                hasReacted
                                  ? 'bg-cyan-500/20 border border-cyan-500/30 text-cyan-300'
                                  : 'bg-slate-800/80 border border-slate-700/50 text-slate-400 hover:border-slate-600'
                              }`}
                            >
                              <span>{emoji}</span>
...
export default function Finnysights() {
  ...
  const fetchComments = useCallback(async (symbol) => { if (!symbol) return; const stockComments = await getStockComments(symbol, 20); setComments(stockComments); const uids = [...new Set(stockComments.map(c => c.uid).filter(Boolean))]; if (uids.length > 0) { getUserAvatars(uids).then(map => setCommentAvatarMap(prev => ({ ...prev, ...map }))); } }, []);
  ...
  useEffect(() => { initRealTime(); fetchCryptoData(); fetchLeaderboard(); fetchNews(); const pollInterval = setInterval(() => { fetchStockPrices(); fetchCryptoData(); }, 60000); return () => { clearInterval(pollInterval); disconnectWebSocket(); }; }, []);
